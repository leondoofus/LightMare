...
public class DragAndDropSystem : FSystem {
    private Family _ddGO = FamilyManager.getFamily(new AllOfComponents(typeof(PointerOver), typeof(DragAndDrop)));
    ...
    /*protected override void onProcess(int familiesUpdateCount) {
        ...
        if (go != null)
        {
            ...
            {
                if (!go2.Equals(go) && go2.GetComponent<DragAndDrop>().rotating && Input.GetMouseButton(0))
                    check = false;
            }
            if (check)
            {
